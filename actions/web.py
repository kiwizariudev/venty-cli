...
def _web_search(query: str, max_results: int = 5) -> str:
    if not _HAS_REQUESTS: return "requests not installed"
    try:
        encoded = urllib.parse.quote(query)
        r = _req.get(f"https://api.duckduckgo.com/?q={encoded}&format=json&no_html=1&skip_disambig=1", timeout=10, headers={"User-Agent": "Venty/1.0"})
        data = r.json()
        lines = []
        if data.get("AbstractText"):
            lines.append(f"Summary: {data['AbstractText']}")
            if data.get("AbstractURL"): lines.append(f"Source : {data['AbstractURL']}")
        for item in data.get("RelatedTopics", [])[:max_results]:
            if isinstance(item, dict) and item.get("Text"): lines.append(f"• {item['Text']}")
...
def _web_post(url: str, body: str) -> str:
    ...
    except Exception as e: return f"POST error: {e}"
...
ACTIONS = {
    ...
    "web_get_json": {
        ...
        "execute": lambda a: _stdout(json.dumps(_req.get(a[0], timeout=10).json(), indent=2)[:2000] if _HAS_REQUESTS else "requests not installed"),
    },
}
